...
phi = readmatrix(filePath,'Sheet','大规模衰减',"Range","B2:Q2"); % U2 e4 m10
U_phi = phi(1:2);
e_phi = phi(3:6);
m_phi = phi(7:16);
h = readmatrix(filePath,'Sheet','小规模瑞丽衰减',"Range","B2:Q2");
U_h = h(1:2);
e_h = h(3:6);
m_h = h(7:16);
taskflow = readmatrix(filePath,'Sheet','任务流',"Range","B2:Q2");
...
U_total_score = sum(U_score)
e_total_score = sum(e_score)
m_total_score = sum(m_score)

total_score = U_total_score + e_total_score + m_total_score  %20 20 10
